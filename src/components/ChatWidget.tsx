...
export default function ChatWidget() {
  ...
  return (
    <>
      {/* Chat panel */}
      {open && (
        <div className="fixed bottom-20 right-5 z-40 w-[360px] max-w-[calc(100vw-2.5rem)] flex flex-col rounded-2xl border border-neutral-700 bg-neutral-950 shadow-2xl overflow-hidden">
          {/* Header */}
          <div className="flex items-center justify-between px-4 py-3 border-b border-neutral-800 bg-neutral-900">
            <div className="flex items-center gap-2">
              <span className="w-2 h-2 rounded-full bg-green-400 animate-pulse" />
              <span className="text-sm font-medium text-white">Ask about Fares</span>
            </div>
            <button
          ...
          <div className="flex-1 overflow-y-auto px-4 py-4 flex flex-col gap-3" style={{ maxHeight: '360px' }}>
            ...
            {loading && (
              <div className="flex justify-start">
                <div className="bg-neutral-800 rounded-xl rounded-bl-sm px-3.5 py-2.5">
              ...
              <div className="flex flex-col gap-1.5 mt-1">
                {SUGGESTIONS.map((s) => (
                  <button
                    key={s}
                    onClick={() => send(s)}
                    className="text-left text-xs text-neutral-400 border border-neutral-700 hover:border-blue-500 hover:text-blue-400 rounded-lg px-3 py-2 transition-colors"
                  >
                    {s}
                  </button>
                ))}
          ...
          <div className="px-3 py-3 border-t border-neutral-800 flex items-center gap-2">
            <input
              ...
              disabled={loading}
              className="flex-1 bg-neutral-800 text-sm text-white placeholder-neutral-500 rounded-lg px-3 py-2 outline-none focus:ring-1 focus:ring-blue-500 disabled:opacity-50 transition"
            />
            <button
              onClick={() => send()}
